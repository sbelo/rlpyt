...
def build_samples_buffer(agent, env, batch_spec, bootstrap_value=False,
    ...
    T, B = batch_spec
    if (not agent.is_player) and env.serial:
        T = T*env.obs_size
...
def get_example_outputs(agent, env, examples, subprocess=False):
    ...
    if agent.is_player:
        if env.serial:
            for ii in range(env.obs_size):
                env.step(env.observer_action_space.sample())
        else:
            env.step(env.observer_action_space.sample())
        a = env.player_action_space.sample()
        env.step(a)
        if env.serial:
            for ii in range(env.obs_size):
                o, r, d, env_info = env.step(env.observer_action_space.sample())
        else:
            o, r, d, env_info = env.step(env.observer_action_space.sample())
    else:
        if env.serial:
            for ii in range(env.obs_size):
                a = env.observer_action_space.sample()
                env.step(a)
        else:
            a = env.observer_action_space.sample()
            env.step(a)
        o, r, d, env_info = env.step(env.player_action_space.sample())
    r = np.asarray(r, dtype="float32")  # Must match torch float dtype here.
    agent.reset()
    agent_inputs = torchify_buffer(AgentInputs(o, a.astype('float32'), r))
