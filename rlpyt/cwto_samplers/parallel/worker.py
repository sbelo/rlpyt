...
def sampling_process(common_kwargs, worker_kwargs):
    ...
    collector = c.CollectorCls(
        rank=w.rank,
        envs=envs,
        player_samples_np=w.player_samples_np,
        observer_samples_np=w.observer_samples_np,
        batch_T=c.batch_T,
        TrajInfoCls=c.TrajInfoCls,
        agent=c.get("agent", None),  # Optional depending on parallel setup.
        player_sync=w.get("player_sync", None),
        observer_sync=w.get("observer_sync", None),
        player_step_buffer_np=w.get("player_step_buffer_np", None),
        observer_step_buffer_np=w.get("observer_step_buffer_np", None),
        global_B=c.get("global_B", 1),
        env_ranks=w.get("env_ranks", None),
    )
    player_agent_inputs, player_traj_infos, observer_agent_inputs, observer_traj_infos = collector.start_envs(c.max_decorrelation_steps)
    ...
    if c.get("eval_n_envs", 0) > 0:
        ...
        eval_collector = c.eval_CollectorCls(
            ...
            envs=eval_envs,
            TrajInfoCls=c.TrajInfoCls,
            player_traj_infos_queue=c.player_eval_traj_infos_queue,
            observer_traj_infos_queue=c.observer_eval_traj_infos_queue,
            max_T=c.eval_max_T,
            agent=c.get("agent", None),
            player_sync=w.get("player_sync", None),
            observer_sync=w.get("observer_sync", None),
            player_step_buffer_np=w.get("player_eval_step_buffer_np", None),
            observer_step_buffer_np=w.get("observer_eval_step_buffer_np", None),
        )
    else:
        eval_envs = list()
    ...
    while True:
        collector.reset_if_needed(player_agent_inputs, observer_agent_inputs)  # Outside barrier?
        ...
        if ctrl.do_eval.value:
            ...
            player_agent_inputs, player_traj_infos, player_completed_infos, observer_agent_inputs, observer_traj_infos, observer_completed_infos = collector.collect_batch(
                player_agent_inputs, observer_agent_inputs, player_traj_infos, observer_traj_infos, ctrl.itr.value)
            for info in player_completed_infos:
                c.player_traj_infos_queue.put(info)
            for info in observer_completed_infos:
                c.observer_traj_infos_queue.put(info)
        ctrl.barrier_out.wait()
