...
from rlpyt.cwto_samplers.parallel.base import ParallelSamplerBase
...
class CpuSampler(ParallelSamplerBase):
    ...
    def obtain_samples(self, itr):
        ...
        class's method.
        """
        self.agent.player.sync_shared_memory()  # New weights in workers, if needed.
        self.agent.observer.sync_shared_memory()  # New weights in workers, if needed.
        return super().obtain_samples(itr)

    def evaluate_agent(self, itr):
        """Like in ``obtain_samples()``, first sync agent shared memory."""
        self.agent.player.sync_shared_memory()
        self.agent.observer.sync_shared_memory()
