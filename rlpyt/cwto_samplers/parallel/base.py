...
from rlpyt.cwto_samplers.base import BaseSampler
from rlpyt.cwto_samplers.buffer import build_samples_buffer
from rlpyt.cwto_samplers.parallel.worker import sampling_process
...
class ParallelSamplerBase(BaseSampler):
    ...
    def initialize(
        ...
        player_examples, observer_examples = self._build_buffers(env, bootstrap_value)
        env.close()
        del env
        ...
        return player_examples, observer_examples  # e.g. In case useful to build replay buffer.
    ...
    def evaluate_agent(self, itr):
        ...
        self.player_sync.stop_eval.value = False
        self.observer_sync.stop_eval.value = False
        self.ctrl.barrier_in.wait()
        player_traj_infos = list()
        observer_traj_infos = list()
        if self.eval_max_trajectories is not None:
            while True:
                time.sleep(EVAL_TRAJ_CHECK)
                player_traj_infos.extend(drain_queue(self.player_eval_traj_infos_queue,
                    guard_sentinel=True))
                observer_traj_infos.extend(drain_queue(self.observer_eval_traj_infos_queue,
                                                     guard_sentinel=True))
                if len(player_traj_infos) >= self.eval_max_trajectories:
                    self.player_sync.stop_eval.value = True
                    self.observer_sync.stop_eval.value = True
        ...
        return player_traj_infos, observer_traj_infos
    ...
    def _agent_init(self, agent, env, global_B=1, env_ranks=None):
        agent.player.initialize(env.spaces()[0], share_memory=True,
            global_B=global_B, env_ranks=env_ranks)

        agent.observer.initialize(env.spaces()[1], share_memory=True,
                                global_B=global_B, env_ranks=env_ranks)
        self.agent = agent

    def _build_buffers(self, env, bootstrap_value):
        self.player_samples_pyt, self.player_samples_np, player_examples = build_samples_buffer(
            self.agent.player, env, self.batch_spec, bootstrap_value,
            agent_shared=True, env_shared=True, subprocess=False)
        self.observer_samples_pyt, self.observer_samples_np, observer_examples = build_samples_buffer(
            self.agent.observer, env, self.batch_spec, bootstrap_value,
            agent_shared=True, env_shared=True, subprocess=False)
    ...
    def _build_parallel_ctrl(self, n_worker):
        ...
        self.player_traj_infos_queue = mp.Queue()
        self.observer_traj_infos_queue = mp.Queue()
        self.player_eval_traj_infos_queue = mp.Queue()
        self.observer_eval_traj_infos_queue = mp.Queue()
        self.player_sync = AttrDict(stop_eval=mp.RawValue(ctypes.c_bool, False))
        self.observer_sync = AttrDict(stop_eval=mp.RawValue(ctypes.c_bool, False))
    ...
    def _assemble_common_kwargs(self, affinity, global_B=1):
        common_kwargs = dict(
            ...
            player_traj_infos_queue=self.player_traj_infos_queue,
            observer_traj_infos_queue=self.observer_traj_infos_queue,
            ctrl=self.ctrl,
        ...
        if self.eval_n_envs > 0:
            common_kwargs.update(dict(
                ...
                player_eval_traj_infos_queue=self.player_eval_traj_infos_queue,
                observer_eval_traj_infos_queue=self.observer_eval_traj_infos_queue,
                )
            )
    ...
    def _assemble_workers_kwargs(self, affinity, seed, n_envs_list):
        ...
        for rank in range(len(n_envs_list)):
            ...
            worker_kwargs = dict(
                ...
                n_envs=n_envs,
                player_samples_np=self.player_samples_np[:, slice_B],
                observer_samples_np=self.observer_samples_np[:, slice_B],
                player_sync=self.player_sync,  # Only for eval, on CPU.
                observer_sync=self.observer_sync,  # Only for eval, on CPU.
            )
            i_env += n_envs
            g_env += n_envs
